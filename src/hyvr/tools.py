...
def surface_gauss_regression(x:np.array,y:np.array,mean:np.float64,variance:np.float64,corl:np.array,dataset:np.array, error:np.array):
    '''
    ...
    x = x.ravel()
    y = y.ravel()
    x_dist = pdist(x, 'sqeuclidean')
    y_dist = pdist(y, 'sqeuclidean')

    # distance of the training points:
    x_t = dataset[:,0].ravel()
    y_t = dataset[:,1].ravel()
    z_t = dataset[:,2].ravel()
    x_dist_t = pdist(x_t, 'sqeuclidean')
    y_dist_t = pdist(y_t, 'sqeuclidean')
    ...
    L_y = scipy.linalg.solve_triangular(L, z_t, lower=True)
