import numpy as np
import numba
from hyvr.utils import azimuth_to_counter_clockwise, get_alternating_facies, normal_plane_from_dip_dip_dir, \
    coterminal_angle, is_point_inside_ellipsoid, dip_dip_dir_bulbset
#from libc.math cimport sin, cos, ceil, acos, sqrt

@numba.jit(nopython=True, parallel=True)
def trough(x,y,z,
           center_coords,dims,azim,
           facies,
    ...
    gross_limit_logic = (x >= xmin) & (x <= xmax) & (y >= ymin) & (y <= ymax) & (z >= zmin) & (z <= zmax)
    ...
    if bulb:
        dip_output,dip_dir_output,norm_distance = dip_dip_dir_bulbset(x,y,z, x_c, y_c, z_c, a, b, c, alpha,dip)
        ...
        if internal_layering:
            ...
            shift = layer_dist + normal_vector[0]*x_c + normal_vector[1]*y_c + normal_vector[2]*z_c
            # X = xmax - xmin
            # Y = ymax - ymin
            # Z = c
            ...
            plane_dist = x * normal_vector[0] + \
                         y * normal_vector[1] + \
                         z * normal_vector[2] - shift
    ...
    # #gross_limit_logic = gross_limit_logic.reshape(x_copy.shape)
    facies_final = np.ones(gross_limit_logic.shape,dtype = np.int32)*(-1)
    facies_final[gross_limit_logic] = facies_output
    #np.where(gross_limit_logic, facies_output,np.ones(gross_limit_logic.shape)*-1)
    facies_final = np.reshape(facies_final,original_shape)
    dip_final = np.empty(gross_limit_logic.shape,dtype=np.float64)
    dip_final[gross_limit_logic] = dip_output
    #dip_final= np.where(gross_limit_logic, dip_output,np.nan)
    dip_final = np.reshape(dip_final, original_shape)
    dip_dir_final = np.empty(gross_limit_logic.shape,dtype=np.float64) 
    dip_dir_final = np.reshape(dip_dir_final, original_shape)
    return facies_final, dip_final ,dip_dir_final#, dip_final, dip_dir_final
